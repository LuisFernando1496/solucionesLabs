...
class ReportController extends Controller
{
    ...
    public function creditsReportDownload(Request $request){
        $from =  $request->from;
        $to = $request->to;
        ...
        view()->share(["bySale" => $creditsBySale,
            "user" => Auth::user(),
            "byClient" =>$creditsTotalByClient,
            "total"=>$creditsTotal,
            "to" => $to,
        ...
        $pdf = PDF::loadView('reports.reportCredits');
      
            // download PDF file with download method
        return $pdf->download('reporteCreditos.pdf');
            
    }

    public function creditsReportDownloadXLS(Request $request){
        return Excel::download(new CreditsExport($request), 'ReporteDeCredito.xlsx');
    }
    ...
    public function purchaseReportDownload(Request $request){
        ...
        view()->share(["purchases" => $purchasesByDate,
        "user" => Auth::user(),
        "total"=>$total,
        "to" => $to,
        "from" =>$showFrom]);

        $pdf = PDF::loadView('reports.reportPurchases');
      
            // download PDF file with download method
        return $pdf->download('compras.pdf');
    }

    public function purchaseReportDownloadXLS(Request $request){
        return Excel::download(new PurchasesExport($request), 'compras.xlsx');
    }

    public function accountStatus($id){

        $creditsByClient = Sale::join('product_in_sales', 'sales.id', '=', 'product_in_sales.sale_id')
        ->join('products', 'product_in_sales.product_id', '=', 'products.id')
        ->select('products.name as producto', 'product_in_sales.sale_price as precio', 'sales.id as folio', 'sales.created_at as fecha')
        ->where('sales.status_credit', '=', 'adeudo')
        ->where('sales.status', '=', true)
        ->where('sales.client_id', '=', $id)->get();

        $totalDebt = Sale::select('sales.cart_total')
        ->where('sales.status_credit', '=', 'adeudo')
        ->where('sales.status', '=', true)
        ->where('sales.client_id', '=', $id)->sum('cart_total');

        $payments = Payment::join('sales', 'sales.id', '=', 'payments.sale_id')
        ->select('sales.id as folio', 'payments.deposit as pago', 'payments.leftover as restante', 'payments.created_at as fecha')
        ->where('sales.status_credit', '=', 'adeudo')
        ->where('sales.status', '=', true)
        ->where('sales.client_id', '=', $id)->get();
        ...
        view()->share(["credits" => $creditsByClient,
        "client" => $client,
        "total"=>$totalDebt,
        "payments" => $payments]);

        $pdf = PDF::loadView('reports.accountStatus');
      
            // download PDF file with download method
        return $pdf->download('estado de cuenta '.$client->name.' '.$client->last_name.'.pdf');
    }
